 ...
 *          否则，会出现：Exception in thread "线程一" Exception in thread "线程二" java.lang.IllegalMonitorStateException
 *    3、wait()，notify()，notifyAll() 三个方法是定义在 java.lang.Object类中
 *
 * 面试题：sleep() 和 wait()的异同？
 *      1、相同点：一旦执行方法，都可以使得当前线程进入阻塞状态。
 *      2、不同点：1）两个方法声明的位置不同：Thread类中声明sleep() , Object类中声明wait()
 *                2) 调用的范围不同：sleep()可以在任何需要的场景下调用。 wait()必须使用在同步代码块 或 同步方法 中
